...
void WriteBlockRow(pqxx::connection& conn, const BlockInsertData& data)
{
    ...
    w.exec(
        pqxx::zview{
            "INSERT INTO blocks ("
            "hash, merkle_root, time, median_time, height, subsidy, "
            "transactions_count, version, status, bits, nonce, difficulty, "
            "chain_work, outputs_count, inputs_count, total_output_value, total_input_value, total_fees, "
            "total_size, total_vsize, total_weight, fee_rates, output_data, input_data, "
            "transaction_data, output_script_types, input_script_types, "
            "output_legacy_signature_operations, input_legacy_signature_operations, input_p2sh_signature_operations, input_witness_signature_operations, "
            "outputs_total_size, inputs_total_size, net_utxo_size_impact, hash_prev_block, network, "
            "nonstandard_create_count, pubkey_create_count, pubkeyhash_create_count, scripthash_create_count, multisig_create_count, "
            "null_data_create_count, witness_v0_keyhash_create_count, witness_v0_scripthash_create_count, witness_v1_taproot_create_count, witness_unknown_create_count, "
            "nonstandard_spend_count, pubkey_spend_count, pubkeyhash_spend_count, scripthash_spend_count, multisig_spend_count, "
            "null_data_spend_count, witness_v0_keyhash_spend_count, witness_v0_scripthash_spend_count, witness_v1_taproot_spend_count, witness_unknown_spend_count, coinbase, "
            "ordinals_weight, ordinals_count, ordinals_size, ordinals_vsize, ordinals_fees, "
            "non_ordinals_weight, non_ordinals_count, non_ordinals_size, non_ordinals_vsize, non_ordinals_fees"
            ") VALUES ("
...
BlockToSql::BlockToSql(const interfaces::BlockInfo& block_info, const CBlockIndex& block_index, script_verify_flags flags)
{
    ...
    for (std::size_t transaction_index = 0; transaction_index < block.vtx.size(); ++transaction_index) {
        ...
        for (std::size_t output_vector = 0; output_vector < transaction->vout.size(); ++output_vector) {
            ...
            addresses_string_stream << network << ","; // network
