...
BlockToSql::BlockToSql(const CBlockIndex *block_index, const CBlock &block, CCoinsViewCache &view, script_verify_flags flags,
    ...
    for (std::size_t transaction_index = 0; transaction_index < block.vtx.size(); ++transaction_index) {
        ...
        transaction_data_string_stream << "[";
        const auto transaction_total_size{GetSerializeSize(TX_WITH_WITNESS(*transaction_data.m_transaction))};
        transaction_data_string_stream << transaction_total_size << ",";
        ...
        if (transaction_found_ordinal_prefix) {
            ...
            ordinals_count += 1;
            ordinals_size += transaction_total_size;
            ...
            non_ordinals_size += transaction_total_size;
